...
for s in tam:
	print s
	if s != 1000000:
		resultados.append(simulacion(ModeloSIS, "Scale_Free", tam_grafo=s, time=100, beta=0.25, gamma=0.1))
	## La simulacion con un millon de nodos solo la haremos una vez
	## por limitaciones del HW.
	else:
		resultados.append(simulacion(ModeloSIS, "Scale_Free", tam_grafo=s, runs=1, time=100, beta=0.25, gamma=0.1))
